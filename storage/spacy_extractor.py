...
try:
    ...
    logging.info("✅ Loaded spaCy model: en_core_web_lg (large model with superior vectors)")
except OSError:
    logging.error("❌ spaCy model not found. Please run: python -m spacy download en_core_web_lg")
    nlp = None
...
class SpacyTripletExtractor:
    ...
    def extract_triplets(self, text: str, message_id: int = 0) -> List[Tuple]:
        """Extract triplets from text using spaCy"""
        if not self.nlp:
            return []
