...
class CriticAgent(BaseAgent):
    ...
    def _critique_mathematical_statement(self, statement: str) -> str:
        """Provide critical analysis of a mathematical statement."""
        try:
            if self.symbolic_engine.is_symbolic_query(statement):
                result = self.symbolic_engine.evaluate(statement)
                if result.get('success'):
                    return (
                        f"🔍 **Mathematical Analysis**: {statement}\n\n"
                        f"**Evaluation**: Expression is valid and computable\n"
                        f"**Result**: {result['result']}\n"
                        f"**Method**: {result['method']}\n"
                        f"**Confidence**: {result['confidence']}\n\n"
                        "**Potential issues**: None identified - mathematical expression is well-formed."
                    )
                else:
                    return (
                        f"🔍 **Mathematical Analysis**: {statement}\n\n"
                        "**Critical issues identified**:\n"
                        f"- Expression evaluation failed: {result.get('error', 'Unknown error')}\n"
                        "- May contain syntax errors or unsupported operations\n"
                        "- Consider reviewing the mathematical notation"
                    )
            else:
                return (
                    f"🔍 **Analysis**: {statement}\n\n"
                    "**Critical observation**: Statement doesn't appear to be a valid mathematical expression.\n"
                    "**Recommendation**: Clarify the intended mathematical operation or provide proper notation."
                )
        except Exception as e:
            return f"🔍 **Analysis Error**: Could not evaluate mathematical statement: {e}"
