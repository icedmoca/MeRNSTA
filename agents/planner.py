...
class PlannerAgent(BaseAgent):
    ...
    def _plan_mathematical_task(self, task: str) -> str:
        """Evaluate a mathematical expression concisely using the symbolic engine.

        Returns only the final answer for a clean chat experience,
        while storing method/confidence for the Thoughts view.
        """
        try:
            result = self.symbolic_engine.evaluate(task) if self.symbolic_engine else {"success": False}
            if result.get("success"):
                # Expose meta for UI thoughts
                self.last_method = result.get("method")
                self.last_confidence = result.get("confidence")
                return str(result.get("result"))
            # Fall back to a brief failure message instead of verbose plan
            return "I couldn't compute that."
        except Exception as e:
            logging.error(f"[PlannerAgent] Math evaluation error: {e}")
            return "I couldn't compute that."
    ...
    def _build_planning_prompt(self, message: str, memory_context: list, context: Dict[str, Any] = None) -> str:
        ...
        raw = "\n".join(prompt_parts)
        # Enforce token budget for planning prompts
        try:
            ...
            return truncate_by_tokens(raw, budget)
        except Exception:
            return raw
